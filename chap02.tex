\chapter{Analysis}
In the previous chapter, we described the key goals of this thesis, focusing on developing a reference documentation generator. 
To fulfill the goals, we must first address the fundamental challenges related to generating documentation for .NET applications.

One of the most important aspects to consider is that .NET is a managed environment, so the documentation  generators can be classified into 2 categories depending on what their input is:
...
A generator using source code as an input, for instance, Doxygen, may seem straightforward; however, it typically has the following disadvantages:
\begin{enumerate}
    \item Language coupling -- The generator is coupled to the given language.
    Adding support for a new language requires the creation of a new parser for it, which is a complex task. Note that this fact interferes with R5. (TODO: pridat odkaz)
...
On the other hand, a generator that uses intermediate code as its input, such as SHFB, is typically easier to develop and doesn't suffer from the issues mentioned earlier either.
Such generators aren't limited to a single language; in the case of .NET, a single generator can handle e.g. C\#, F\#, and Visual Basic programs. 
...
Using this information, we can refine our primary objective -- to develop a reference
...
Since our generator will use the assembly, we will describe the compilation process in .NET in a little bit more detail.
...
First of all, it is necessary to load the assembly that is going to be documented, which is done using the \texttt{Assembly.Load} method.
...
Firstly, it is worth noting that the vast majority of .NET languages use structured XML comments, well known from C\#, to document types and their members.
